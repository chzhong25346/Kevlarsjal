...
def create_dbengine(db='tsxci_daily_db'):
    ...
    tsxci_simulation_db = cfg['mysql']['tsxci_simulation_db']
    ...
    if (db=='tsxci_report'):
        ...
        logger.debug('connecting to %s', url)
    if (db=='tsxci_simulation'):
        url = 'mysql://{0}:{1}@{2}:{3}/{4}?charset=utf8'.format(db_username,db_password,db_ip,db_port,tsxci_simulation_db)
        logger.debug('connecting to %s', url)
